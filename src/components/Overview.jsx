    ...
    <div className="overview-panel" style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div className="overview-content-sections" style={{ display: 'flex', flexDirection: 'column', gap: '45px', marginBottom: '80px' }}>
        ...
        <section className="info-section">
          ...
          <div className="section-text" style={{ fontSize: '15px', color: 'var(--color-text-gray)', lineHeight: '1.7', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <p>
              During a feedforward pass, input features pass sequentially through the network. For each hidden layer, the 
              inputs are multiplied by the connection strengths (weights) and adjusted by an offset value (bias).
            </p>
            <p>
              This combined value is then processed by a non-linear activation function (like ReLU, Tanh, or Sigmoid), which acts 
              as a threshold filter to decide if the signal is strong enough to be sent forward. Introducing these non-linear filters 
              is crucial: without them, a multi-layer network collapses into a single simple equation, unable to solve complex 
              classification tasks. The final layer output forms the model's target prediction.
            </p>
          </div>
        </section>
        ...
        <section className="info-section">
          ...
          <div className="section-text" style={{ fontSize: '15px', color: 'var(--color-text-gray)', lineHeight: '1.7', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <p>
              To train the model, we use an error score (known as a loss function) to quantify how far the network's predictions 
              deviate from correct target answers. Minimizing this error requires calculating feedback signals for every single weight 
              and bias in the network by tracing the error backward through the layers.
            </p>
            <p>
              These feedback signals indicate how much each parameter contributed to the mistake. Optimization algorithms (like SGD, 
              Momentum, or Adam) then use these signals to adjust the connection strengths in a direction that reduces the error. 
              By repeating this forward-loss-backpropagation loop across many training cycles, the network weights iteratively converge 
              towards their optimal configuration, minimizing total prediction error.
            </p>
          </div>
        </section>
